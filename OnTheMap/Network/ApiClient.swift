...
class ApiClient {
    ...
    func getConfiguredTaskForGET(
        ...
        andCompletionHandler handler: @escaping ([String: AnyObject]?, RequestError?) -> Void
        ) -> URLSessionDataTask? {
    ...
    private func getConfiguredDataTask(
        ...
        parameters: [String: String],
        jsonBody: Data?,
        withCompletionHandler handler: @escaping ([String: AnyObject]?, RequestError?) -> Void,
        ...
        guard let url = getURL(fromPath: path, andParameters: parameters) else {
            assertionFailure("The passed url is not a valid one - fix it!")
            return nil
        }
    ...
    private func deserializeJson(from data: Data) -> [String: AnyObject]? {
